...
def _invoke_chat(backends, system_prompt, user_message):
    """Try each backend in priority order; return (review_text, backend) for whichever succeeds."""
    for i, backend in enumerate(backends):
        print(f"🤖 Trying {backend.kind} backend (model={backend.model})")
        try:
            text = _call_backend(backend, system_prompt, user_message)
            return text, backend
...
def _backend_label(backend):
    """Human-readable label for a backend, used in PR comments."""
    if backend.kind == "anthropic":
        return f"Claude (`{backend.model}`)"
    return f"`{backend.model}` via OpenRouter"
...
def post_review_comment(gh_client, repo_owner, repo_name, pr_number, review_text, backend):
    """Post the review as a GitHub comment, naming the backend that produced it."""
    repo = gh_client.get_repo(f"{repo_owner}/{repo_name}")
    pr = repo.get_pull(pr_number)
    label = _backend_label(backend)

    comment_body = f"""## 🤖 AI Code Review — {label}

{review_text}

---
*This review was automatically generated by {label}. Please review and use your judgment when applying suggestions.*"""

    pr.create_issue_comment(comment_body)
    print(f"✅ Posted review comment to PR #{pr_number} (backend={backend.kind}, model={backend.model})")


def post_unavailable_comment(gh_client, repo_owner, repo_name, pr_number, reason):
    """Post a soft-fail comment when every backend is unreachable after retries."""
    repo = gh_client.get_repo(f"{repo_owner}/{repo_name}")
    pr = repo.get_pull(pr_number)
    comment_body = f"""## 🤖 AI Code Review — Skipped

All configured AI backends were unavailable after {MAX_RETRIES + 1} attempts ({reason}).

Re-run the **AI Code Review** workflow on this PR to try again.
...
def main():
    ...
    try:
        ...
        # Get the review
        print("🤔 Getting the review...")
        review_text, used_backend = review_code_with_claude(
            diff_content, total_changes, repo_path=".", proto_context=proto_context
        )

        # Post the review
        print("📤 Posting review to GitHub...")
        post_review_comment(gh_client, repo_owner, repo_name, pr_number, review_text, used_backend)
